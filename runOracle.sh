  ...
  if [ -d "$SCRIPTS_ROOT" ] && [ -n "$(ls -A $SCRIPTS_ROOT)" ]; then
    ...
    for f in $SCRIPTS_ROOT/*; do
        case "$f" in
            ...
            *.dmp)    echo "$0: running $f"; echo "exit" | su -p oracle -c "$ORACLE_HOME/bin/imp system/$ORACLE_PWD file=$f fromuser=ff,fd,fd_admin touser=ff,fd,fd_admin"; echo ;;
...
if [ "$?" == "0" ]; then
   ...
   runUserScripts $ORACLE_BASE/scripts/setup
    ...
    sqlplus system/$ORACLE_PWD@XE @/u01/app/oracle/scripts/Preconfig.sql
